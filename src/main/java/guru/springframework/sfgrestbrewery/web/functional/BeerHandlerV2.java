...
public class BeerHandlerV2 {
    ...
    private final Validator validator; //standard spring validator
    ...
    public Mono<ServerResponse> saveNewBeer(ServerRequest request){
        Mono<BeerDto> beerDtoMono =  request
                .bodyToMono( BeerDto.class)
                .doOnNext(this::validate);
    ...
    private void validate(BeerDto beerDto){
        ...
        if (errors.hasErrors()){
            throw new ServerWebInputException(errors.toString());//Withing the webflux framework when a webserver produces an input exception
            //that will cause a bad request exception
        }
    ...
    public Mono<ServerResponse> deleteBeer(ServerRequest request) {
        return beerService.reactiveDeleteById(Integer.valueOf(request.pathVariable("beerId")))
                ...
                .onErrorResume(e -> e instanceof NotFoundException,
                        e -> ServerResponse.notFound().build());//this statement means
        //that if an error is thrown and the type is 'NotFoundException' the a response with 404 'not found' is returned
    }
}
